...
VERSION = "0.1-ENHANCED-OVERLAPPING"
  ...
  Start decoding SSTV signal at 50.5 seconds into the audio
    ...
    def _enhance_image(self, img):
        try:
            ...
            # Minimal softening filter
            img = img.filter(ImageFilter.GaussianBlur(radius=0.5))
            
            # Apply noise reduction
            #img = img.filter(ImageFilter.MedianFilter(size=3))
            
            # Strong sharpening (crisp details without ringing)
            enhancer = ImageEnhance.Sharpness(img)
            img = enhancer.enhance(1.80)
...
class SSTVDecoder(object):
    ...
    def __init__(self, audio_file, slant=0.0):
        ...
        self._freq_offset = 0  # For frequency tracking
        self._slant = slant
        ...
        try:
            ...
            # SUPER ENHANCED: Bandpass filter to isolate SSTV tones (1000-2500 Hz)
            # This single step dramatically improves decoding on noisy/poor recordings
            self._samples = self._bandpass_filter(self._samples, self._sample_rate)
        
        except wave.Error as e:
            raise ValueError(f"Error reading WAV file: {e}")
            
    def _bandpass_filter(self, data, fs):
        """Zero-phase Butterworth bandpass 1000-2500 Hz"""
        if len(data) < 100:
            return data
        lowcut, highcut, order = 1000.0, 2500.0, 4
        nyq = 0.5 * fs
        low = lowcut / nyq
        high = highcut / nyq
    ...
    def _peak_fft_freq(self, data):
        """QSSTV-OPTIMIZED: Overlapping FFT windows for better frequency accuracy"""

        if len(data) < 64:
            # Fallback for very small segments
        ...
        # QSSTV approach: Use multiple overlapping windows and average
        window_size = min(512, len(data))
        overlap = window_size // 2
        
        peak_freqs = []
        for start in range(0, len(data) - window_size, overlap):
            window_data = data[start:start + window_size]
            windowed_data = window_data * hann(len(window_data))
            fft = np.abs(np.fft.rfft(windowed_data))
        ...
        if peak_freqs:
            # Use median to reject outliers (QSSTV technique)
            result = np.median(peak_freqs)
            # Frequency tracking: update offset for calibration
            self._freq_offset = self._freq_offset * 0.9 + (result - 1500) * 0.1
            return result
        else:
            # Fallback to single FFT
            windowed_data = data * hann(len(data))
            fft = np.abs(np.fft.rfft(windowed_data))
            x = np.argmax(fft)
            peak = barycentric_peak_interp(fft, x)
    ...
    def _find_header_from(self, search_start):
        ...
        for current_sample in range(search_start, len(self._samples) - header_size,
            ...
            if (abs(self._peak_fft_freq(leader_1_area) - 1900) < 60
               and abs(self._peak_fft_freq(break_area) - 1200) < 60
               and abs(self._peak_fft_freq(leader_2_area) - 1900) < 60
               and abs(self._peak_fft_freq(vis_start_area) - 1200) < 60):
