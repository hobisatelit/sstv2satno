...
VERSION = "0.1-ENHANCED-BANDPASSFILTER"
...
class SSTVDecoder(object):
    ...
    def _bandpass_filter(self, data, fs):
        ...
        if len(data) < 100:
            return data
        lowcut, highcut, order = 800.0, 2500.0, 4
        nyq = 0.5 * fs
        low = lowcut / nyq
        high = highcut / nyq
    ...
    def decode(self, skip=0.0):
        ...
        while True:
            ...
            try:
                ...
                #print(f"DEBUG: Mode={self.mode.NAME}, COLOR={self.mode.COLOR},CHAN_COUNT={self.mode.CHAN_COUNT}")
    ...
    def _draw_image(self, image_data):
        """Renders the image from the decoded sstv signal - FIXED GBR mapping for Martin modes"""
        ...
        for y in range(height):
            ...
            for x in range(width):
                if channels == 2:
                    if self.mode.HAS_ALT_SCAN and self.mode.COLOR == COL_FMT.YUV:
                        # Robot 36
                    ...
                    if self.mode.COLOR == COL_FMT.GBR:
                        # FIXED: Martin/Scottie GBR transmission order
                        # Transmission: chan[0]=Green, chan[1]=Blue, chan[2]=Red
                        # PIL wants (Red, Green, Blue)
                        pixel = (image_data[y][2][x],   # Red  = transmitted Red (chan 2)
                                 image_data[y][0][x],   # Green = transmitted Green (chan 0)
                                 image_data[y][1][x])   # Blue  = transmitted Blue (chan 1)
